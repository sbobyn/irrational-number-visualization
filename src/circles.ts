...
function drawAxisLabels() {
  ...
  renderer.ctx.fillText(
    "0",
    renderer.toCanvasX(plotWidth) - 10,
    renderer.toCanvasY(0) - 5
  );
  renderer.ctx.fillText(
    `2${piUnicode}`,
    renderer.toCanvasX(plotWidth) - 17,
    renderer.toCanvasY(0) + 15
  );
}
...
function draw() {
  ...
  renderer.drawLine(
    0,
    0,
    armLen * Math.cos(theta2Rate * theta),
    armLen * Math.sin(theta2Rate * theta)
  );
  renderer.drawPoint(
    armLen * Math.cos(theta2Rate * theta),
    armLen * Math.sin(theta2Rate * theta)
