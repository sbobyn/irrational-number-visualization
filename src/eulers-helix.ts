...
cosineGeometry.setAttribute(
  "position",
  new THREE.BufferAttribute(cosineVertices, 3)
);
sineGeometry.setAttribute(
  "position",
  new THREE.BufferAttribute(sineVertices, 3)
);
helixGeometry.setAttribute(
  "position",
  new THREE.BufferAttribute(helixVertices, 3)
);
...
const arrowHelix = new THREE.ArrowHelper(
  dirHelix,
...
function update() {
  ...
  const newDir = new THREE.Vector3(
    Math.cos(freq * thetas[0] + phase) * amp,
    Math.sin(freq * thetas[0] + phase) * amp,
    0
  );
  newDir.normalize();
