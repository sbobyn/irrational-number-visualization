...
let theta1 = initTheta1, theta2 = initTheta2;
let deltaTheta = 0.01;
let theta2Rate = Math.PI;
let numInnerRotations = 0;
let numOuterRotations = 0;
const innerRotSpan = document.getElementById("numInnerRotations");
const outerRotSpan = document.getElementById("numOuterRotations");
let prevX = 0;
let prevY = 0;
let xs = new Float64Array(3);
let ys = new Float64Array(3);
const deltaThetaInput = document.getElementById("deltaSlider");
const deltaThetaSpan = document.getElementById("deltaValue");
deltaThetaInput.oninput = () => {
    deltaTheta = parseFloat(deltaThetaInput.value);
    deltaThetaSpan.textContent = deltaTheta.toFixed(3);
};
deltaThetaInput.oninput(null);
...
function drawMainCanvas() {
    ...
    for (let i = 0; i < prevXs.length - 1; i++) {
        mainRenderer.ctx.moveTo(mainRenderer.toCanvasX(prevXs[i]), mainRenderer.toCanvasY(prevYs[i]));
        mainRenderer.ctx.lineTo(mainRenderer.toCanvasX(prevXs[i + 1]), mainRenderer.toCanvasY(prevYs[i + 1]));
    }
...
function drawZoomCanvas() {
    ...
    for (let i = 0; i < prevXs.length - 1; i++) {
        ...
        zoomRenderer.ctx.lineTo(toZoomCanvasX(prevXs[i + 1]), toZoomCanvasY(prevYs[i + 1]));
    }
...
update();
main();
//# sourceMappingURL=irrational-vis.js.map