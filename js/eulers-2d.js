...
const sineRenderer = new Renderer(canvas, plotWidth, plotHeight, 0, canvas.height / 2);
sineRenderer.cw = canvas.width * (2 / 3);
sineRenderer.ch = canvas.height / 2;
const circleRenderer = new Renderer(canvas, 1, 1, canvas.width * (2 / 3), canvas.height / 2);
...
function drawSine() {
    ...
    for (let i = 0; i < thetas.length; i++) {
        ...
        sineRenderer.ctx.lineTo(sineRenderer.toCanvasX(x), sineRenderer.toCanvasY(y));
    }
    sineRenderer.ctx.stroke();
    sineRenderer.ctx.fillStyle = "red";
    sineRenderer.drawPoint(Math.PI, Math.sin(Math.PI + phase));
    cosArcRenderer.ctx.fillText(`y = sin(${thetaUnicode})`, 5, canvas.height / 2 + 15);
}
function drawCosine() {
    ...
    for (i; i < thetas.length; i++) {
        ...
        cosineRenderer.ctx.lineTo(cosineRenderer.toCanvasX(x), cosineRenderer.toCanvasY(y));
    }
...
function drawCircle() {
    ...
    circleRenderer.ctx.fillText(`z(${thetaUnicode}) = x(${thetaUnicode}) + iy(${thetaUnicode})`, canvas.width * (2 / 3) + 5, 15);
    circleRenderer.drawArc(0, 0, circleRenderer.cw / (2 * plotHeight), -(Math.PI + phase), true);
    ctx.lineWidth = 1;
...
function drawCosArc() {
    ...
    cosArcRenderer.drawArc(-1, -1, cosArcRenderer.cw * 0.5 * (1 + Math.cos(Math.PI + phase) / plotHeight), -Math.PI / 2, true);
}
function drawBorders() {
    canvasRenderer.ctx.strokeStyle = "black";
    canvasRenderer.drawLine(-plotWidth, 0, plotWidth, 0);
    canvasRenderer.drawLine(plotWidth / 3, -plotHeight, plotWidth / 3, plotHeight);
}
function drawGrid() {
    ...
    // cosine arc
    // horizontal
    cosArcRenderer.drawArc(-1, -1, cosArcRenderer.cw * 0.5 * (1 + Math.cos(Math.PI) / plotHeight), -Math.PI / 2, true);
    cosArcRenderer.drawArc(-1, -1, cosArcRenderer.cw * 0.5, -Math.PI / 2, true);
    cosArcRenderer.drawArc(-1, -1, cosArcRenderer.cw * (1 - 0.5 * (1 + Math.cos(Math.PI) / plotHeight)), -Math.PI / 2, true);
    // circle
    // horizontal
    ...
    if (-phase + (3 / 2) * Math.PI < Math.PI) {
        cosineRenderer.drawLine(-phase + (3 / 2) * Math.PI, -1, -phase + (3 / 2) * Math.PI, 1);
        sineRenderer.drawLine(-phase + (3 / 2) * Math.PI, -1, -phase + (3 / 2) * Math.PI, 1);
    }
    if (-phase - (3 / 2) * Math.PI > -Math.PI) {
        cosineRenderer.drawLine(-phase - (3 / 2) * Math.PI, -1, -phase - (3 / 2) * Math.PI, 1);
        sineRenderer.drawLine(-phase - (3 / 2) * Math.PI, -1, -phase - (3 / 2) * Math.PI, 1);
...
draw();
//# sourceMappingURL=eulers-2d.js.map